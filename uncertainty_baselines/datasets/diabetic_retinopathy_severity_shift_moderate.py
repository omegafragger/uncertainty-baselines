...
class DiabeticRetinopathySeverityShiftModerateDataset(base.BaseDataset):
  ...
  def __init__(self,
    ...
    super(DiabeticRetinopathySeverityShiftModerateDataset, self).__init__(
        ...
        cache=cache)
    logging.info(
        'Building Diabetic Retinopathy Severity Shift dataset with moderate '
        'decision threshold.')
  ...
  def _create_process_example_fn(self) -> base.PreProcessFn:
    ...
    def _example_parser(example: Dict[str, tf.Tensor]) -> Dict[str, tf.Tensor]:
      ...
      # We place the decision threshold between 1 and 2
...
class DiabeticRetinopathySeverityShiftModerate(tfds.core.GeneratorBasedBuilder):
  ...
  Allows for the formation of an out-of-distribution dataset with
  ...
  def _generate_examples_helper(self,
    ...
    if is_in_domain is not None and csv_path:
      in_domain_severity_levels = {0, 1, 2}
      ood_severity_levels = {3, 4}
      severity_level_set = (
